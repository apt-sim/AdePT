                          ...
                          int numTracks = -1)                  // only print those with index < numTracks
{
  ...
  if (numTracks != numLive) {
    std::cout << " Printing " << numTracks << " out of " << numLive << " live tracks." << std::endl;
  }
  ...
  for (int i = 0; i < lastTrack; i++) {
    ...
    if (trk.status == alive) {
      if (++numPrinted < numTracks) trk.print(i, verbose);

      if (trk.index == 0) trk.index = trk.mother_index * 100 + i;
    }
  }
}
#endif
