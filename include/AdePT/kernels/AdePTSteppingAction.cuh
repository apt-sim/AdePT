...
void KillTrack(bool& alive, double& eKin, double& edep, LeakStatus& leak) {
  if (!alive) return;
...
struct NoAction {
  ...
  __device__ __forceinline__
  static void ElectronAction(bool&, double&, double&, LeakStatus&,
                               vecgeom::Vector3D<double> const&, double const&, int const&,
                               G4HepEmData const*, Params const&) {}

  __device__ __forceinline__
  static void GammaAction(bool&, double&, double&, LeakStatus&,
                            vecgeom::Vector3D<double> const&, double const&, int const&,
                            G4HepEmData const*, Params const&) {}
};
...
struct CMSAction {
  ...
  __device__ __forceinline__
  static void AllParticleCheck(bool& alive, double& eKin, double& edep, LeakStatus& leak,
  ...
  __device__ __forceinline__
  static void ElectronAction(bool& alive, double& eKin, double& edep, LeakStatus& leak,
                               vecgeom::Vector3D<double> const& pos,
                               double const& globalTime,
                               int const& mcIndex,
                               G4HepEmData const* g4HepEmData,
                               Params const& params)
  {
    if (!alive) return;
  ...
  __device__ __forceinline__
  static void GammaAction(bool& alive, double& eKin, double& edep, LeakStatus& leak,
                            vecgeom::Vector3D<double> const& pos,
                            double const& globalTime,
                            int const& /*mcIndex*/,
                            G4HepEmData const* /*g4HepEmData*/,
...
struct LHCbAction {
                               ...
                               Params const& params)
  {
    ...
    if (pos.x() > params.xmax || pos.x() < params.xmin ||
        pos.y() > params.ymax || pos.y() < params.ymin ||
        pos.z() > params.zmax || pos.z() < params.zmin ||
        eKin < params.ecut) {
      KillTrack(alive, eKin, edep, leak);
    }
  }
