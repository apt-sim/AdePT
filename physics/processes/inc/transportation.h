...
template<class  fieldPropagator_t,
         bool   BfieldOn = true>
...
__host__ __device__
float transportation<fieldPropagator_t, BfieldOn>
::transport( track             & mytrack,
             fieldPropagator_t & fieldPropagator,
             float               physics_step )
{
  ...
  if( step < physics_step )
     mytrack.current_process = BfieldOn ? -2 : -1;
