...
public:
  __host__ __device__ IterationStats_impl() { initialise(); }

  __host__ __device__ void initialise()
  {
    maxDone.store(0);
    totalIterations.store(0);
  }

  // __host__
  __device__ static IterationStats_impl *MakeInstanceAt(void *addr)
  {
...
__global__ void MakeInstanceAt_glob(char *buffer_dev, IterationStats_impl **ISdev_PtrHolder)
{
...
public:
  ...
  __host__ /*unsigned*/ int GetTotal()
  {                   /*unsigned*/
    int totalNow = 0; // Was GetTotalFromDevice()
...
inline IterationStats::IterationStats()
{
