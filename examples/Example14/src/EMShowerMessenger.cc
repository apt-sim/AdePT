...
{
  fDirectory = new G4UIdirectory("/example14/fastSim/");
  fDirectory->SetGuidance("Set mesh parameters for the example fast sim model.");

  fPrintCmd = new G4UIcmdWithoutParameter("/example14/fastSim/print", this);
  fPrintCmd->SetGuidance("Print current settings.");

  fNbOfHitsCmd = new G4UIcmdWithAnInteger("/example14/fastSim/numberOfHits", this);
