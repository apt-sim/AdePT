...
def compare_csv(file1, file2, n1, n2, tol=0.01, plot_file=None):
    ...
    # Print results
    if failed_layers:
        print(f"The results are not reproducible. Relative errors exceed {tol}% in the following layers:")
        for layer, err, val1, val2 in failed_layers:
            print(f"Layer {layer}: Relative Error = {err:.6f}%, File1 = {val1:.6f}, File2 = {val2:.6f}")
        sys.exit(1)
    else:
        print(f"The results are reproducible. All layers have relative errors within {tol}%.")
