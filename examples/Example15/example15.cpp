...
const G4VPhysicalVolume *InitGeant4(const std::string &gdml_file)
{
  ...
  if (world == nullptr) {
    std::cerr << "example15: World volume not set properly check your setup selection criteria or GDML input!\n";
    return world;
  }
...
int main(int argc, char *argv[])
{
  ...
  example15(particles, energy, batch, MCCindex, &scoringPerVolume, &globalScoring, NumVolumes, NumPlaced, &hepEmState,
            rotatingParticleGun);
