 ...
 *
 * The first time it is called, it saves the random seed of Geant4.
 * Subsequently, it offsets this seed by the EventID, and reseeds
 * Geant4's random generator.
 * This ensures that every event has the same random sequence, irrespective
 * of which thread it runs on.
 *
 * Afterwards, events are either generated by the ParticleGun or by reading
 * a HepMC3 file.
 */
void ParticleGun::GeneratePrimaries(G4Event *aEvent)
{
  // Ensure that same random numbers are used irrespective of
  // which thread picks this event
  if (fInitialSeed == 0) {
