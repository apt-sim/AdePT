...
__device__ void initOneTrack(unsigned int index, uint64_t rngBase, track &aTrack, unsigned int eventId)
{
...
__global__ void initTracks(adept::BlockData<track> *trackBlock, unsigned int numTracks, unsigned int eventId,
...
__global__ void overwriteTracks(adept::BlockData<track> *trackBlock, unsigned int numTracks, unsigned int eventId,
                                unsigned int runId = 0)
{
  /* initialize the tracks with random particles */
  int pclIdx = blockIdx.x * blockDim.x + threadIdx.x;
