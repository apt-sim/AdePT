...
struct AllParticleQueues {
  ...
  Interactions interactions[ParticleType::NumParticleTypes];
};

// Kernel to initialize the set of queues per particle type.
__global__ void InitParticleQueues(ParticleQueues queues, Interactions interactions, size_t Capacity)
{
  adept::MParray::MakeInstanceAt(Capacity, queues.currentlyActive);
  adept::MParray::MakeInstanceAt(Capacity, queues.nextActive);
  adept::MParray::MakeInstanceAt(Capacity, interactions.queues[0]);
  adept::MParray::MakeInstanceAt(Capacity, interactions.queues[1]);
  adept::MParray::MakeInstanceAt(Capacity, interactions.queues[2]);
}
...
__global__ void FinishIteration(AllParticleQueues all, Stats *stats)
{
  for (int i = 0; i < ParticleType::NumParticleTypes; i++) {
    ...
    all.interactions[i].queues[0]->clear();
    all.interactions[i].queues[1]->clear();
    all.interactions[i].queues[2]->clear();
            ...
            G4HepEmState *state, GunConfig gunConfig)
{
  ...
  for (int i = 0; i < ParticleType::NumParticleTypes; i++) {
    ...
    COPCORE_CUDA_CHECK(cudaMalloc(&particles[i].interactions.queues[0], QueueSize));
    COPCORE_CUDA_CHECK(cudaMalloc(&particles[i].interactions.queues[1], QueueSize));
    COPCORE_CUDA_CHECK(cudaMalloc(&particles[i].interactions.queues[2], QueueSize));
    InitParticleQueues<<<1, 1>>>(particles[i].queues, particles[i].interactions, Capacity);
    ...
    do {
      ...
      if (numElectrons > 0) {
        ...
        TransportElectrons<<<transportBlocks, ThreadsPerBlock, 0, electrons.stream>>>(
            ...
            electrons.interactions, globalScoring, scoringPerVolume);
        ...
        IonizationEl<<<transportBlocks, ThreadsPerBlock, 0, interactionStreams[0]>>>(
            electrons.tracks, electrons.interactions.queues[0], secondaries, electrons.queues.nextActive, globalScoring,
            scoringPerVolume);
        BremsstrahlungEl<<<transportBlocks, ThreadsPerBlock, 0, electrons.stream>>>(
            electrons.tracks, electrons.interactions.queues[1], secondaries, electrons.queues.nextActive, globalScoring,
            scoringPerVolume);
      ...
      if (numPositrons > 0) {
        ...
        TransportPositrons<<<transportBlocks, ThreadsPerBlock, 0, positrons.stream>>>(
            ...
            positrons.interactions, globalScoring, scoringPerVolume);
        ...
        IonizationPos<<<transportBlocks, ThreadsPerBlock, 0, interactionStreams[1]>>>(
            positrons.tracks, positrons.interactions.queues[0], secondaries, positrons.queues.nextActive, globalScoring,
            scoringPerVolume);
        BremsstrahlungPos<<<transportBlocks, ThreadsPerBlock, 0, positrons.stream>>>(
            positrons.tracks, positrons.interactions.queues[1], secondaries, positrons.queues.nextActive, globalScoring,
            scoringPerVolume);
        AnnihilationPos<<<transportBlocks, ThreadsPerBlock, 0, interactionStreams[2]>>>(
            positrons.tracks, positrons.interactions.queues[2], secondaries, positrons.queues.nextActive, globalScoring,
            scoringPerVolume);
      ...
      if (numGammas > 0) {
        ...
        TransportGammas<<<transportBlocks, ThreadsPerBlock, 0, gammas.stream>>>(
            gammas.tracks, gammas.queues.currentlyActive, secondaries, gammas.queues.nextActive, gammas.interactions,
        ...
        PairCreation<<<transportBlocks, ThreadsPerBlock, 0, interactionStreams[0]>>>(
            gammas.tracks, gammas.interactions.queues[0], secondaries, gammas.queues.nextActive, globalScoring,
            scoringPerVolume);
        // About 10% of all gammas:
        ComptonScattering<<<transportBlocks, ThreadsPerBlock, 0, interactionStreams[1]>>>(
            gammas.tracks, gammas.interactions.queues[1], secondaries, gammas.queues.nextActive, globalScoring,
            scoringPerVolume);
        // About 15% of all gammas:
        PhotoelectricEffect<<<transportBlocks, ThreadsPerBlock, 0, interactionStreams[2]>>>(
            gammas.tracks, gammas.interactions.queues[2], secondaries, gammas.queues.nextActive, globalScoring,
            scoringPerVolume);
        for (auto i = 0; i < 3; ++i) {
      ...
      AllParticleQueues queues = {
          {electrons.queues, positrons.queues, gammas.queues},
          {electrons.interactions, positrons.interactions, gammas.interactions},
      };
  ...
  for (int i = 0; i < ParticleType::NumParticleTypes; i++) {
    ...
    COPCORE_CUDA_CHECK(cudaFree(particles[i].interactions.queues[0]));
    COPCORE_CUDA_CHECK(cudaFree(particles[i].interactions.queues[1]));
    COPCORE_CUDA_CHECK(cudaFree(particles[i].interactions.queues[2]));
