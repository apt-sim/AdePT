                                ...
                                ScoringPerVolume *scoringPerVolume)
{
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < activeSize; i += blockDim.x * gridDim.x) {
    ...
    interactions.queues[winnerProcessIndex]->push_back(slot);
    survive(false);
  }
}

template <int ProcessIndex>
__device__ void GammaInteraction(Track *particles, const adept::MParray *active, Secondaries secondaries,
...
__global__ void PairCreation(Track *particles, const adept::MParray *active, Secondaries secondaries,
                             ...
                             ScoringPerVolume *scoringPerVolume)
{
  GammaInteraction<0>(particles, active, secondaries, activeQueue, globalScoring, scoringPerVolume);
}
__global__ void ComptonScattering(Track *particles, const adept::MParray *active, Secondaries secondaries,
                                  adept::MParray *activeQueue, GlobalScoring *globalScoring,
                                  ScoringPerVolume *scoringPerVolume)
{
  GammaInteraction<1>(particles, active, secondaries, activeQueue, globalScoring, scoringPerVolume);
}
__global__ void PhotoelectricEffect(Track *particles, const adept::MParray *active, Secondaries secondaries,
                                    adept::MParray *activeQueue, GlobalScoring *globalScoring,
                                    ScoringPerVolume *scoringPerVolume)
{
  GammaInteraction<2>(particles, active, secondaries, activeQueue, globalScoring, scoringPerVolume);
}
