...
{
  fExampleDir = new G4UIdirectory("/adeptint/");
  fExampleDir->SetGuidance("UI commands specific to this example");

  fDetectorDir = new G4UIdirectory("/adeptint/detector/");
  fDetectorDir->SetGuidance("Detector construction UI commands");

  fAdeptDir = new G4UIdirectory("/adeptint/adept/");
  fDetectorDir->SetGuidance("AdePT integration UI commands");

  fPrintCmd = new G4UIcmdWithoutParameter("/adeptint/detector/print", this);
  fPrintCmd->SetGuidance("Print current settings.");

  fFileNameCmd = new G4UIcmdWithAString("/adeptint/detector/filename", this);
  fFileNameCmd->SetGuidance("Set GDML file name");
  //
  fRegionNameCmd = new G4UIcmdWithAString("/adeptint/detector/regionname", this);
  fRegionNameCmd->SetGuidance("Set fast simulation region name");
  //
  fSensVolNameCmd = new G4UIcmdWithAString("/adeptint/detector/addsensitivevolume", this);
  fSensVolNameCmd->SetGuidance("Add a sensitive volume to the list");
  //
  fSensVolGroupCmd = new G4UIcmdWithAString("/adeptint/detector/sensitivegroup", this);
  fSensVolGroupCmd->SetGuidance("Define a wildcard for a sensitive volumes group");
  //
  fActivationCmd = new G4UIcmdWithABool("/adeptint/adept/activate", this);
  fActivationCmd->SetGuidance("(Activate AdePT");
  //
  fVerbosityCmd = new G4UIcmdWithAnInteger("/adeptint/adept/verbose", this);
  fVerbosityCmd->SetGuidance("Verbosity level for AdePT integration transport");

  fBufferThresholdCmd = new G4UIcmdWithAnInteger("/adeptint/adept/threshold", this);
  fBufferThresholdCmd->SetGuidance("Threshold for starting AdePT transport");

  fTrackSlotsCmd = new G4UIcmdWithAnInteger("/adeptint/adept/milliontrackslots", this);
  fTrackSlotsCmd->SetGuidance("Total number of allocated track slots per GPU");

  fFieldCmd = new G4UIcmdWith3VectorAndUnit("/adeptint/detector/setField", this);
