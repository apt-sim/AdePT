...
{
  fGunDir = new G4UIdirectory("/adept/gun/");
  fGunDir->SetGuidance("gun control");

  fHepmcCmd = new G4UIcmdWithoutParameter("/adept/gun/hepmc", this);
  fHepmcCmd->SetGuidance("select hepmc input");
  fHepmcCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  fDefaultCmd = new G4UIcmdWithoutParameter("/adept/gun/setDefault", this);
  ...
  fRndmDirCmd = new G4UIcmdWithADouble("/adept/gun/rndmDir", this);
  ...
  fRandomizeGunCmd = new G4UIcmdWithABool("/adept/gun/randomizeGun", this);
  ...
  fAddParticleCmd = new G4UIcmdWithAString("/adept/gun/addParticle", this);
  fAddParticleCmd->SetGuidance("When using randomization, add a particle to the list of possibilities\n\
                                Usage: /adept/gun/addParticle type [\"weight\" weight] [\"energy\" energy unit]\n\
                                type: particle name\n\
  ...
  fMinPhiCmd = new G4UIcmdWithADoubleAndUnit("/adept/gun/minPhi", this);
  ...
  fMaxPhiCmd = new G4UIcmdWithADoubleAndUnit("/adept/gun/maxPhi", this);
  ...
  fMinThetaCmd = new G4UIcmdWithADoubleAndUnit("/adept/gun/minTheta", this);
  ...
  fMaxThetaCmd = new G4UIcmdWithADoubleAndUnit("/adept/gun/maxTheta", this);
