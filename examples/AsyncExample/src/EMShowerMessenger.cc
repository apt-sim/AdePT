...
{
  fDirectory = new G4UIdirectory("/adeptint/fastSim/");
  fDirectory->SetGuidance("Set mesh parameters for the example fast sim model.");

  fPrintCmd = new G4UIcmdWithoutParameter("/adeptint/fastSim/print", this);
  fPrintCmd->SetGuidance("Print current settings.");

  fNbOfHitsCmd = new G4UIcmdWithAnInteger("/adeptint/fastSim/numberOfHits", this);
