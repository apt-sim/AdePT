...
std::shared_ptr<AdePTTransportInterface> AdePTConfiguration::CreateAdePTInstance(unsigned int nThread)
{
  ...
  if (sFactoryFunction == nullptr) {
    throw std::logic_error{"No factory function has been registered to create AdePT instances."};
  }

  auto adept =
      sFactoryFunction(fNThread, 1024 * 1024 * GetMillionsOfTrackSlots(), 1024 * 1024 * GetMillionsOfHitSlots(),
