...
struct RegisterFunctionPointer {
  RegisterFunctionPointer()
  {
    G4cout << "Using thread-local AdePT backend.\n";
    AdePTConfiguration::SetAdePTFactoryFunction(AdePTTransportFactory);
  }
  ~RegisterFunctionPointer() { AdePTConfiguration::SetAdePTFactoryFunction(nullptr); }
};
static RegisterFunctionPointer registerFuncRAII{};
