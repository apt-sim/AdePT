...
# run test
$ADEPT_EXECUTABLE --do_validation --accumulated_events -m ${CI_TMP_DIR}/reproducibility.mac --output_dir ${CI_TMP_DIR} --output_file cms_ttbar_split

# If there are two files in CI_TMP_DIR, compare the results
if [ "$(ls -1 ${CI_TMP_DIR} | wc -l)" -eq 2 ]; then
    # allow for small rounding error of 1e-6 due to summation per thread
    $CI_TEST_DIR/python_scripts/check_reproducibility.py --file1 ${CI_TMP_DIR}/cms_ttbar_monolithic.csv \
                                                        --file2 ${CI_TMP_DIR}/cms_ttbar_split.csv \
                                                        --tol 1e-5
fi